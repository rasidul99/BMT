...
export default function SafeDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border pb-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="p-2 border border-border rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition flex items-center justify-center text-xs"
          >
          ...
          <button
            ...
            className="bg-card hover:bg-muted text-foreground border border-border text-xs font-semibold px-3.5 py-2 rounded-lg transition shadow-xs flex items-center gap-1.5"
          >
      ...
      {/* 2. Top 5 Core Metrics Cards (Per Client Specification) */}
      <div className="grid gap-3.5 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
        {/* Card 1: Accounts */}
        <div className="border border-border bg-card p-4 rounded-xl shadow-xs space-y-2 relative overflow-hidden group hover:border-blue-500/50 transition">
          ...
          <div>
            <div className="text-2xl font-black text-foreground">{metrics.totalAccounts}</div>
        ...
        <div className="border border-border bg-card p-4 rounded-xl shadow-xs space-y-2 relative overflow-hidden group hover:border-sky-500/50 transition">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
              Post Scheduled
            </span>
          ...
          <div>
            <div className="text-2xl font-black text-sky-600 dark:text-sky-400">{metrics.postScheduled}</div>
        ...
        {/* Card 3: Success */}
        <div className="border border-border bg-card p-4 rounded-xl shadow-xs space-y-2 relative overflow-hidden group hover:border-emerald-500/50 transition">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
              Success
            </span>
          ...
          <div>
            <div className="text-2xl font-black text-emerald-600 dark:text-emerald-400">{metrics.success}</div>
        ...
        {/* Card 4: In Process */}
        <div className="border border-border bg-card p-4 rounded-xl shadow-xs space-y-2 relative overflow-hidden group hover:border-amber-500/50 transition">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
              In Process
            </span>
          ...
          <div>
            <div className="text-2xl font-black text-amber-600 dark:text-amber-400">{metrics.inProcess}</div>
        ...
        {/* Card 5: Failed */}
        <div className="border border-border bg-card p-4 rounded-xl shadow-xs space-y-2 relative overflow-hidden group hover:border-rose-500/50 transition">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
              Failed
            </span>
          ...
          <div>
            <div className="text-2xl font-black text-rose-600 dark:text-rose-400">{metrics.failed}</div>
            <p className="text-[10px] text-muted-foreground font-medium mt-0.5">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 border border-border bg-card rounded-xl p-5 shadow-xs space-y-4">
          ...
          <div className="space-y-2.5">
            {filteredJobs.length === 0 ? (
              ...
              filteredJobs.map((job) => (
                <div
                  key={job.id}
                  className="border border-border p-3.5 rounded-lg hover:bg-muted/30 transition flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-xs"
                >
                  <div className="space-y-1 flex-1">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-foreground">{job.variationTitle}</span>
                      {job.type && (
                    ...
                    <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                      <span className="font-medium text-blue-600 dark:text-blue-400">{job.accountName}</span>
                      <span>•</span>
                      <span>{job.scheduledFor}</span>
                      {job.lastError && (
                        <>
                          <span>•</span>
                          <span className="text-rose-500 font-semibold">{job.lastError}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {/* Actions: Retry or Publish Now */}
                    {job.status === "Failed" && (
                      <button
                        onClick={() => retryFailedJob(job.id)}
                        className="p-1.5 bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 rounded-md transition flex items-center gap-1 font-semibold text-[10px]"
                    ...
                    {job.status === "Pending" && (
                      <button
                        ...
                        className="p-1.5 bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 dark:text-blue-400 rounded-md transition flex items-center gap-1 font-semibold text-[10px]"
        ...
        <div className="border border-border bg-card rounded-xl p-5 shadow-xs space-y-4 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="space-y-2.5">
              {accounts.map((acc) => (
                <div
                  key={acc.id}
                  className="border border-border p-3 rounded-lg flex items-center justify-between text-xs hover:bg-muted/30 transition"
                >
          ...
          <button
            onClick={() => router.push(`/workspace/${workspaceId}/safe/connect-accounts`)}
            className="w-full py-2 bg-muted hover:bg-muted/80 text-foreground border border-border rounded-lg text-xs font-semibold transition flex items-center justify-center gap-1.5"
          >
            <PlusCircle className="w-3.5 h-3.5" />
